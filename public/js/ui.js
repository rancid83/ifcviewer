import { loadIFC, applyColor, resetColor, applyPosition, resetPosition } from '/js/viewer.js';

// 페이지 로드 시 자동으로 IFC 파일 로드
window.addEventListener('DOMContentLoaded', async () => {
  try {
    const response = await fetch('/files/KIT-Simple-Road-Test-Web-IFC4x3_RC2.ifc');
    if (response.ok) {
